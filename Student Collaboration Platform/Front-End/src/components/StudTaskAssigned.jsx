...
export default function StudTaskAssigned(props) {
    ...
    const handleMyTask = (id) =>{
        localStorage.setItem("taskid",id)
        // console.log(typeof(parseInt(JSON.parse(localStorage.getItem('taskid'))))) 
    ...
    useEffect(()=>{
        try {
            ...
            setUserId(taskid)
            const res = UserService.getAssignedTaskById(taskid)
            res.then((data)=>{
                setTasks(data)  
            })
            if(tasks==null){
            ...
            <Box  sx={{ m: 5, display: 'flex-box', gap: 5 ,flexWrap:"wrap",width:"90%"}}>
                    ...
                    
                {
                    tasks?.map((project, id) => (
                        <div className="task-assigned" onClick={() => handleMyTask(project.tid)}>
                        <Card sx={{ width: 345, height: 180 }}>
                            <CardActionArea>
                            <CardHeader
                                title={project.task}

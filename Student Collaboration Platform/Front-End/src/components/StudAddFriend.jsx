...
export default function StudAddFriends() {
    ...
    const handleAddFriend = (fid) =>{
        ...
        // console.log(fid,uid);
        const res = UserService.addFriend(uid,fid)
        res.then(data => {
            const upateRes = UserService.getAllNonFriends(uid)
            upateRes.then(dat => {
                setUsers(dat)
                console.log(users);
                if(dat==0)
            ...
            <Box  sx={{ width:'100%', mt: '0px', height: 'calc(100vh - 95px)' }}>
                <Box  sx={{ m: 5, display: 'flex-box', gap: 5 ,flexWrap:"wrap",width:"90%"}}>
                    ...
                    <Box sx={{position:'relative',width:'100%', mt: '0px',m:5,flexDirection:'row', display: 'flex', gap: 5 ,flexWrap:"wrap"}}>
                    {
                        ...
                        <Button variant='contained'sx={{mt:5,mr:2,height:'35%',borderRadius:'50px' ,":hover":{backgroundColor:'red'}}} onClick={()=>{handleAddFriend(friend.uid)}} endIcon={<PersonAddIcon/>} >
                             ADD 
                        </Button>
                    </Card>

...
export default function TeachGradedTask() {
    ...
    useEffect(()=>{
        try {
            ...
            setUserId(taskid)
            const res = UserService.getGradedProjects(taskid)
            res.then((data)=>{
                setTasks(data)  
            })
            if(tasks==null){
                    ...
                    
                {
                    tasks?.map((project, id) => (
                        ...
                        <Card sx={{ width: 345, maxHeight: 150 }}>
                            ...
                            />
                            <CardContent>
                                <Typography variant="body2">Grade: {project.grade}</Typography>
                            </CardContent>
                            </CardActionArea>
                        </Card>
                        </div>
        ...
        :
            ...
            <div className="no-project-content" style={{ width:'100%', mt: '0px',display:'flex',flexDirection:'column',backgroundColor:""}}>
                ...
                <div className="project-content" style={{textAlign:'center',position:'relative',marginTop:'25px',color:'rgb(var(--zpOnDataLight))'}}>
                    ...
                    <Button  sx={{color:'white',backgroundColor:'red',margin:'10px', padding:'10px',":hover":{backgroundColor:'blue'}}} >Go to My-Project </Button>
                </div>
            </div>
